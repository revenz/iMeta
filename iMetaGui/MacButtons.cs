...
namespace iMetaGui
{
	...
	public partial class MacButtons : Gtk.Bin
	{
		public MacButtons ()
		{
			this.Build ();
		...
		void HandleEventMinimizehandleExposeEvent (object o, ExposeEventArgs args)
		{
			...
			{
				pixbuf = Images.Mac_MinimizeButton_Hover;
			}
			else
			{	
				pixbuf = Images.Mac_MinimizeButton;		
			}
		...
		void HandleEventMaximizehandleExposeEvent (object o, ExposeEventArgs args)
		{
			...
			{
				pixbuf = Images.Mac_MaximizeButton_Hover;
			}
			else
			{	
				pixbuf = Images.Mac_MaximizeButton;		
			}
            pixbuf.RenderPixmapAndMask( out pixmap, out pixmap_mask, 255); 
		...
		{		
			...
			if(this.ExitHover)
			{
				pixbuf = Images.Mac_CloseButton_Hover;
			}
			else
			{	
				pixbuf = Images.Mac_CloseButton;		
			}
            pixbuf.RenderPixmapAndMask( out pixmap, out pixmap_mask, 255); 
